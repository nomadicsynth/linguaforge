...
import warnings
from MLflowCustomCallback import MLflowCustomCallback
...
parser = argparse.ArgumentParser(description="Train a text-generation model using the SFTTrainer.")
...
# Add the arguments to continue training from a checkpoint
parser.add_argument("--continue_training", action="store_true", help="Continue training from a saved model")
parser.add_argument("--checkpoint_dir", type=str, default=None, help="Directory to load the model from")
...
if args.continue_training:
    if args.checkpoint_dir is None:
        raise ValueError("Please provide a checkpoint directory to continue training from.")
    results_dir = args.checkpoint_dir
...
training_args = TrainingArguments(
    ...
    report_to="mlflow" if run_hyperparameter_search else [],
...
# Set up the MLflow callback if hyperparameter search is enabled
if run_hyperparameter_search:
    mlflow_callback = MLflowCustomCallback(hp_search="optuna")
